...
const BASE_URL = "http://ec2-52-12-190-52.us-west-2.compute.amazonaws.com:8080/api";

const login = async <T>(endpoint: string, data: Record<string, unknown>, token?: string) => {
    try {
  ...
  export const forgetPassword = async <T>(email: string, password: string, token: string) => {
  ...
  export const changeUserStatus = async <T>(id: string, status: string, token: string) => {
